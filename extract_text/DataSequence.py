...
class DataSequence(object):
    ...
    def chunkstds(self):
        """Splits the stored [data] into the discrete chunks, then takes the std of each chunk
        (this is assuming that [data] is a numpy array) and returns the resulting matrix with
        one row per chunk.
        """
        dsize = self.data.shape[1]
    ...
    def from_grid(cls, grid_transcript, trfile, word_time="middle"):
        ...
        word_starts = np.array([float(str(t)[:5]) for t in word_starts])
        word_ends = np.array([float(str(t)[:5]) for t in word_ends])
        if word_time == "start":
            word_times = word_starts
        elif word_time == "end":
            word_times = word_ends
        elif word_time == "middle":
        ...
        return cls(data, split_inds, word_times, trtimes+tr/2.0)
    ...
    def from_chunks(cls, chunks):
        ...
        data = list(itools.chain(*list(map(list, chunks)))) ## 19.6ms for 10k 6-w chunks
