...
DEFAULT_BAD_WORDS = frozenset(["sentence_start", "sentence_end", "br", "lg", "ls", "ns", "sp", "ns_ap", "cg", "sl", "ig"])
...
def make_semantic_model(ds, embedding, model_name="word2vec"):
    """ Given a DataSequence and word embedding object return the word vectors.

    Parameters
    ----------
    ds : DataSequence
        DataSequence object where every word is assigned to a presentation time.
    embedding : word embedding object
        This can be word2vec or fastText object
    model_name : str
    ...
    -------
    new_ds : DataSequence
        DataSequence object where every word is assigned to a word vector representation.
    """
    newdata = []
    not_found = []
    for w in ds.data:
        if model_name == "word2vec":
            try:
                vec = embedding.get_vector(w)
            except KeyError as e:
                not_found.append(w)
                vec = np.zeros((embedding.vector_size,))
        newdata.append(vec)
    print("Embedding for words {} not found".format(set(not_found)))
    new_ds = DataSequence(np.array(newdata), ds.split_inds, ds.data_times, ds.tr_times)
    return new_ds
